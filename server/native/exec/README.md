...
the JS prelude (`server/src/jobs/runtime/prelude.ts`), so capabilities are a TS
...
`server/src/exec/native/`. Bun embeds that loader and addon in compiled binaries.
