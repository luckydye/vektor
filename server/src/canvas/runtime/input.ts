/** Pointer samples and gestures, and the pan/zoom controls. */
import type {
  CanvasPoint,
  FitReference,
  ViewportCamera,
...
interface PointerSample<TEvent = PointerEvent> {
  event: TEvent;
  screen: CanvasPoint;
  world: CanvasPoint;
}

interface PointerGesture<TEvent = PointerEvent> extends PointerSample<TEvent> {
...
function pointerSample<TEvent extends { clientX: number; clientY: number }>(
  event: TEvent,
  viewportRect: { left: number; top: number } | null | undefined,
...
// ---------------------------------------------------------------------------
// from input/viewportControls.ts
// ---------------------------------------------------------------------------

interface ViewportZoomLimits {
  minZoom?: number;
  maxZoom?: number;
}

interface PanCameraByScreenDeltaOptions {
...
interface ZoomCameraAtPointOptions extends ViewportZoomLimits {
  camera: ViewportCamera;
  screen: ScreenSize;
  fit: FitReference;
  screenX: number;
...
interface ViewportControlsOptions extends ViewportZoomLimits {
