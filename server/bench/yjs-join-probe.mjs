...
ws.onmessage = (e) => {
  ...
  if (type === T.YjsUpdate) {
    yjsFrames++;
    yjsBytes += u.byteLength;
    if (!firstYjsAt) {
      firstYjsAt = Date.now();
      log(
        `FIRST YjsUpdate after ${firstYjsAt - joinSentAt}ms, ${(u.byteLength / 1048576).toFixed(1)}MB`,
      );
    }
  } else if (type === T.Error) {
    log(`ERROR frame: ${new TextDecoder().decode(u.subarray(1))}`);
  } else {
    log(`recv type=${type}`);
  }
};
    ...
    try {
      ws.send(
        frameJson(T.PresenceJoin, {
          room: DOC,
          clientId: "probe",
          user: { id: "p", name: "probe" },
          state: { kind: "canvas", t: Date.now() },
        }),
      );
    } catch {}
  }
...
setTimeout(() => {
  log(
    `SUMMARY: firstYjs=${firstYjsAt ? firstYjsAt - joinSentAt : "NEVER"}ms yjsFrames=${yjsFrames} yjsMB=${(yjsBytes / 1048576).toFixed(1)}`,
  );
  ws.close();
