// Ported from IronCalc `components/WorksheetCanvas/worksheetCanvas.ts` at tag v0.8.3, MIT OR Apache-2.0.
...
import { attachOutlineHandle } from "./outlineHandle.ts";
import type { Cell } from "./types.ts";
import type { WorkbookState } from "./workbookState.ts";
...
// Read per render rather than once at import: this module is reachable during
// SSR, where there is no `window`, and the ratio changes when the window moves
// to a display with a different density.
function getDevicePixelRatio(): number {
  return (typeof window === "undefined" ? 0 : window.devicePixelRatio) || 1;
}
...
export class WorksheetCanvas {
  ...
  /**
   * Set by `setScrollPosition`: the next render only moves the view, so the
   * wrapped text of each cell is still good. Any other render starts from
   * scratch, since it may be following an edit.
   */
  ...
  getFrozenRowsHeight(): number {
    const frozenRows = this.model.getFrozenRowsCount(this.model.getSelectedSheet());
    if (frozenRows === 0) {
      return 0;
    }
  ...
  getFrozenColumnsWidth(): number {
    const frozenColumns = this.model.getFrozenColumnsCount(this.model.getSelectedSheet());
    if (frozenColumns === 0) {
      return 0;
    }
    let frozenColumnsWidth = 0;
    for (let column = 1; column <= frozenColumns; column += 1) {
      frozenColumnsWidth += this.getColumnWidth(this.model.getSelectedSheet(), column);
    }
  ...
  private computeCellTextLeftRight(
    ...
    if (columnToTheLeft && columnToTheLeft > frozenColumnsCount) {
      ...
      this.computeCellText(row, columnToTheLeft, textX, textY, columnWidth, rowHeight);
    }
    ...
    if (columnToTheRight) {
      ...
      this.computeCellText(row, columnToTheRight, textX, textY, columnWidth, rowHeight);
    }
  }
  ...
  private getCellLines(
    ...
    // A long scroll through a big sheet would otherwise grow this without end
  ...
  private computeCellText(
    ...
    const { horizontal: horizontalAlign, vertical: verticalAlign } = this.getAlignment(
      style,
      cellType,
    );
    ...
    // wrapping past that is invisible work — and a cell can hold a *lot* of text.
    ...
    const ratingIconOffset = extended.rating?.show_value
      ...
      : 0;
    const iconOffset = extended.icon?.show_value ? ICON_AREA_WIDTH : ratingIconOffset;
    ...
    lines.forEach((text, line) => {
      const textWidth = widths[line] as number;
      let textX: number;
      let textY: number;
  ...
  private renderCellStyle(
    ...
    if (this.spills.get(`${row}-${column}`) !== 1) {
      ...
      if (border.left) {
        ...
        const leftExtended = this.model.getCellStyle(selectedSheet, row, column - 1);
      ...
      if (x > headerColumnWidth + 0.51 || border.left) {
        drawBorder(context, borderLeftStyle, borderLeftColor, x, y, x, y + height, true);
      }
    } else {
    ...
    if (border.top) {
      ...
      const topExtended = this.model.getCellStyle(selectedSheet, row - 1, column);
    ...
    if (y > headerRowHeight + 0.51 || border.top) {
      drawBorder(context, borderTopStyle, borderTopColor, x, y, x + width, y, false);
    }
  }
  ...
  private renderCellText(textProperties: TextProperties) {
    ...
    const paneRight =
      column <= frozenColumns ? headerColumnWidth + frozenColumnsWidth : this.width;
    const paneBottom =
  ...
  private addColumnResizeHandle(x: number, column: number, columnWidth: number): void {
    ...
    let resizeHandleUp = (event: MouseEvent): void => {
      ...
      if (newColumnWidth !== columnWidth) {
        this.onColumnWidthChanges(this.model.getSelectedSheet(), column, newColumnWidth);
      }
    };
    ...
    div.addEventListener("dblclick", (event) => {
      ...
      for (const row of rows) {
        ...
        const { style: cellStyle } = this.model.getCellStyle(sheet, row, column);
    ...
    for (let row = firstRow; row <= bottomRightCell.row; row += 1) {
      ...
      context.fillStyle = selected
        ? isFullRowSelected
          ...
          : this.theme.headerSelectedBackground
        : this.theme.headerBackground;
      context.fillRect(0.5, topLeftCornerY + 0.5, headerColumnWidth - 1, rowHeight - 1);
      if (selected) {
      ...
      // A handle of a row scrolled up under the frozen rows would sit on top of
  ...
  private renderColumnHeaders(
    ...
    for (const handle of columnHeaders.querySelectorAll(".column-resize-handle")) {
      handle.remove();
    }
    ...
    // They are added to the DOM *before* the frozen ones — the flex `order`
    // property puts them back in the right place, while the DOM order makes the
    // frozen headers paint on top of a partially scrolled one. The container has
    // `overflow: hidden`, which clips at the row headers.
    ...
    for (let column = 1; column <= frozenColumns; column += 1) {
      const selected = column >= columnStart && column <= columnEnd;
      deltaX += this.addColumnHeader(deltaX, column, selected, isFullColumnSelected, 1);
    }
  ...
  private addColumnHeader(
    ...
    const columnWidth = this.getColumnWidth(this.model.getSelectedSheet(), column);
    if (columnWidth <= 0) {
      return 0;
    }
    const div = document.createElement("div");
  ...
  getAreaDimensions(
    ...
    if (frozenRows !== 0 || frozenColumns !== 0) {
      ...
      if (frozenColumns > 0) {
        xFrozenEnd += this.getColumnWidth(this.model.getSelectedSheet(), frozenColumns);
      }
      if (frozenRows > 0) {
        yFrozenEnd += this.getRowHeight(this.model.getSelectedSheet(), frozenRows);
      }
  ...
  private getColumnWidth(sheet: number, column: number): number {
    return Math.round(this.model.getColumnWidth(sheet, column) * COLUMN_WIDTH_SCALE);
  }
  ...
  private drawCellOutline(): void {
    const { cellArrayStructure, cellOutline, areaOutline, cellOutlineHandle } = this;
    ...
    const [selectedSheet, selectedRow, selectedColumn] = this.model.getSelectedCell();
    const { topLeftCell } = this.getVisibleCells();
    const frozenRows = this.model.getFrozenRowsCount(selectedSheet);
    const frozenColumns = this.model.getFrozenColumnsCount(selectedSheet);
    ...
    const padding = -1;
    const width = this.getColumnWidth(selectedSheet, selectedColumn) + 2 * padding;
    ...
    if (columnStart === columnEnd && rowStart === rowEnd) {
      ...
      // Any outline can stick out into the headers once a row/column is only
      // partially scrolled, so clipping no longer depends on where the
      // selection starts relative to the first visible cell.
  ...
  private drawActiveRanges(topLeftCell: Cell, bottomRightCell: Cell): void {
    ...
    this.clipToGrid();
    ctx.setLineDash([2, 2]);
    const referencedRange = this.workbookState.getEditingCell()?.referencedRange || null;
    if (referencedRange) {
  ...
  renderSheet(): void {
    ...
    context.save();
    context.beginPath();
    context.rect(headerColumnWidth, frozenY, frozenX - headerColumnWidth, this.height);
    context.clip();
    for (let { row } = topLeftCell; row <= bottomRightCell.row; row += 1) {
      x = headerColumnWidth;
      const rowHeight = this.getRowHeight(selectedSheet, row);
