// Ported from IronCalc `components/WorksheetCanvas/outlineHandle.ts` at tag v0.8.3, MIT OR Apache-2.0.
// See ./README.md for what was changed and why.

import { LAST_COLUMN, LAST_ROW } from "./constants.ts";
import { AreaType } from "./workbookState.ts";
import type { WorksheetCanvas } from "./worksheetCanvas.ts";

export function attachOutlineHandle(worksheet: WorksheetCanvas): HTMLDivElement {
  ...
  const resizeHandleUp = (_event: MouseEvent): void => {
    ...
    const selectedColumnEnd = Math.max(columnStart + width - 1, extendedArea.columnEnd);
