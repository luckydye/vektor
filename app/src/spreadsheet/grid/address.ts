// Ported from IronCalc `components/util.ts` at tag v0.8.3, MIT OR Apache-2.0.
// See ./README.md for what was changed and why.

import { columnNameFromNumber, quoteName, type SelectedView } from "@ironcalc/wasm";
import { LAST_COLUMN, LAST_ROW } from "./constants.ts";
import type { Area, Cell } from "./types.ts";
...
const cellToString = (row: number, column: number, absolute: boolean): string =>
  absolute
...
export const getCellAddress = (selectedArea: Area, selectedCell: Cell): string => {
...
export function rangeToStr(
  ...
  const sheetName = range.sheet === referenceSheet ? "" : quoteName(referenceName);
