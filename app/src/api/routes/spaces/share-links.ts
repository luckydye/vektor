...
/**
 * Both scopes a link may take, and both resolve to a page to render. A category
 * has no page of its own, and a space-wide link would be a public grant.
 */
...
/** The optional password on a create request. */
...
/** GET /api/v1/spaces/:spaceId/share-links?documentId= — every link on a page. */
export const GET: ApiRouteHandler = (context) =>
  withApiErrorHandling(async () => {
    ...
    // The same bar as creating one: sharing a page is an editor's to do.
    await verifyAccess(
      spaceId,
...
/**
 * POST /api/v1/spaces/:spaceId/share-links
 * Body: name, resourceType, resourceId, expiresInDays, password?
 */
export const POST: ApiRouteHandler = (context) =>
  withApiErrorHandling(async () => {
    ...
    // Required: a link outlives its creator, so nothing else retires it.
