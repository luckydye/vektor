 ...
 *
 * Only spaces where the user holds a space-wide role are offered or accepted:
 * the exchange mints a space-wide token carrying that role, so a space reached
 * only through a document/tree/category grant is not a valid choice.
 *
 * Query params:
...
/**
 * The spaces a CLI token can actually be minted for: those where the user holds
 * a space-wide role, which is what the token exchange delegates.
 *
 * `listUserSpaces` is deliberately wider — it also returns spaces the user only
 * reaches through a document, document-tree or category grant, with `userRole`
...
export const POST: ApiRouteHandler = (context) =>
  withApiErrorHandling(async () => {
    ...
    // `spaces` only holds spaces where the user has a space-wide role, so a
    // space they merely reach through a document/tree/category grant is refused
    // here rather than turning into a space-wide token at exchange time.
