  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <button
        ...
        @click="handleStartCreateSchedule"
    ...
    >
      <form @submit.prevent="handleCreateSchedule" class="space-y-3">
        ...
        <div class="flex justify-end gap-2">
          <button
            type="button"
            @click="handleCancelCreateSchedule"
          ...
          >
            <td class="px-4 py-2.5">
              <div class="flex items-center gap-2">
                <span
                  :class="schedule.enabled ? 'bg-green-500' : 'bg-neutral-300'"
            ...
            <td class="px-4 py-2.5 whitespace-nowrap">
              <code
                class="px-1.5 py-0.5 text-size-small bg-neutral-100 rounded-sm font-mono"
                >{{ schedule.cronExpression }}</code
              >
              <span v-if="schedule.timezone" class="ml-1 text-size-small text-neutral-400"
                >{{ schedule.timezone }}</span
              >
            </td>
            <td class="px-4 py-2.5 whitespace-nowrap text-neutral-500">
              {{ schedule.enabled && schedule.nextRunAt ? formatDateTime(schedule.nextRunAt) : '—' }}
            </td>
            <td class="px-4 py-2.5 whitespace-nowrap text-neutral-500">
    ...
    <div class="mt-8 pt-6 border-t border-neutral-100">
      ...
      <div
        v-if="isLoadingRuns && runs.length === 0"
        class="text-center py-6 text-size-medium text-neutral-500"
      >
        Loading runs...
      </div>
        ...
        <table class="min-w-full text-size-medium">
          ...
          <tbody class="divide-y divide-neutral-100">
            <template v-for="run in runs" :key="run.id">
              <tr
                class="hover:bg-neutral-50"
                :class="run.error ? 'cursor-pointer' : ''"
              ...
              <tr v-if="expandedRunId === run.id && run.error">
                <td colspan="5" class="px-4 py-2.5 bg-red-50">
                  <p class="text-size-small text-red-700 font-mono break-all">
                    {{ run.error }}
                  </p>
                </td>
              </tr>
