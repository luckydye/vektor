  ...
  <div class="overflow-hidden mb-10">
    <div class="flex items-center justify-between">
      ...
      >
        ...
        <span
          v-if="doc"
          class="text-size-medium font-semibold text-neutral-800 group-hover:text-blue-600 transition-colors"
        >
          {{ docTitle(doc) }}
        </span>
    ...
    <div class="relative overflow-hidden">
      <template v-if="doc && doc.type && doc.type !== 'document'">
        ...
        >
          <span class="text-size-medium font-medium text-neutral-800"
            >{{ docTitle(doc) }}</span
          >
          <span class="ml-auto text-size-small text-neutral-400 capitalize"
            >{{ doc.type }}</span
          >
        </a>
      </template>
