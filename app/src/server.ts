 ...
 * serves. Set on the outgoing Node response up front, so it covers both
 * Hono-handled responses (whose `Response` headers are copied onto `res`
 * afterwards by `sendWebResponse`) and Astro-handled responses (where the
 * Astro handler writes directly to `res`).
 *
