...
const T = { Subscribe: 0, Unsubscribe: 1, Event: 2, Error: 3, YjsJoin: 4, YjsUpdate: 5, PresenceJoin: 6, PresenceUpdate: 7, PresenceLeave: 8, PresenceSnapshot: 9 };
...
const agg = {
  sent: {}, recv: {}, recvBytes: 0,
  latSum: 0, latCount: 0, latMax: 0,
};
function bump(map, type, n = 1) { map[type] = (map[type] ?? 0) + n; }
...
class Client {
  constructor(i) {
    ...
    this.ws.onclose = (e) => { this.closed = true; this.closeCode = e.code; };
    this.ws.onerror = () => { this.errored = true; };
  }
  send(frame, type) { try { this.ws.send(frame); this.sent++; bump(agg.sent, type); } catch { /* buffer full / closed */ } }
  onOpen() {
    if (YJS) this.send(frameYjsJoin(DOC), T.YjsJoin);
    this.send(frameJson(T.PresenceJoin, {
      room: DOC, clientId: this.clientId,
  ...
  cursorState(x, y) {
    return { kind: "canvas", t: Date.now(), pointer: { x, y }, cursorColor: "#3b82f6",
      view: { x: 0, y: 0, scale: 1 }, selectionIds: [], focusedNodeId: null, activeTool: "select" };
  }
  onMessage(e) {
    ...
    if (type === T.PresenceUpdate) {
      try {
        ...
        if (typeof t === "number") {
          ...
          agg.latSum += lat; agg.latCount++; if (lat > agg.latMax) agg.latMax = lat;
        }
      } catch { /* ignore */ }
    }
  }
  tick(x, y) {
    ...
    this.send(frameJson(T.PresenceUpdate, { room: DOC, clientId: this.clientId, state: this.cursorState(x, y) }), T.PresenceUpdate);
  }
}

console.log(`[cfg] host=${HOST} clients=${CLIENTS} rate=${RATE}/s seconds=${SECONDS} yjs=${YJS}`);
...
const stat = setInterval(() => {
  ...
  const stalled = clients.filter((c) => c.joined && !c.closed && now - c.lastRecvAt > 1500 && c.lastRecvAt > 0).length;
  const closed = clients.filter((c) => c.closed).length;
  console.log(`[t+${((now - start) / 1000).toFixed(0)}s] send=${sRate}/s recv=${rRate}/s in=${brate}KB/s avgLat=${avgLat}ms maxLat=${agg.latMax}ms stalled=${stalled} closed=${closed}`);
  lastStat = { sent, recv, bytes: agg.recvBytes, t: now };
}, 1000);

setTimeout(() => {
  clearInterval(driver); clearInterval(stat);
  const sent = clients.reduce((a, c) => a + c.sent, 0);
  const recv = clients.reduce((a, c) => a + c.recv, 0);
  console.log("\n=== SUMMARY ===");
  console.log(`total sent=${sent} recv=${recv} recvMB=${(agg.recvBytes / 1048576).toFixed(1)}`);
  console.log(`sent by type:`, Object.fromEntries(Object.entries(agg.sent).map(([k, v]) => [NAME[k], v])));
  console.log(`recv by type:`, Object.fromEntries(Object.entries(agg.recv).map(([k, v]) => [NAME[k], v])));
  console.log(`latency avg=${agg.latCount ? Math.round(agg.latSum / agg.latCount) : 0}ms max=${agg.latMax}ms samples=${agg.latCount}`);
  console.log(`amplification (recv/sent)=${(recv / Math.max(1, sent)).toFixed(2)}  [~${CLIENTS - 1}x expected for pure fan-out]`);
