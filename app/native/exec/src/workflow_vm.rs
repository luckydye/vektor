...
impl WorkflowVm {
    pub fn new(code: &str, inputs: &Json) -> Result<Self, String> {
        let log_queue: Gc<GcRefCell<VecDeque<String>>> = Gc::new(GcRefCell::new(VecDeque::new()));
    ...
    pub fn step(&mut self) -> VmStep {
        ...
        if let Some(mut job) = self.pending_jobs.borrow_mut().pop_front() {
            ...
            let inputs =
                serde_json::from_str(&job.inputs_json).unwrap_or(Json::Object(Default::default()));
...
mod tests {
    ...
    #[test]
    fn logs_runs_a_job_and_returns_object_output() {
        let mut vm = WorkflowVm::new(
            "log(input.name); const result = await runJob('ext', 'job', { count: 2 }); return { answer: result.value + 1 };",
            &json!({ "name": "start" }),
        )
        .unwrap();

        assert!(matches!(vm.step(), VmStep::Log(message) if message == "start"));
        let pending_id = match vm.step() {
            VmStep::PendingJob {
                id,
                extension_id,
                job_id,
                ...
                assert_eq!(extension_id, "ext");
                assert_eq!(job_id, "job");
                assert_eq!(inputs, json!({ "count": 2 }));
                id
            }
            _ => panic!("expected pending job"),
        };

        vm.resolve_job(&pending_id, json!({ "value": 41 })).unwrap();
        match vm.step() {
            VmStep::Done(output) => {
                assert_eq!(output["answer"].as_f64(), Some(42.0));
            }
            VmStep::Pending => panic!("workflow remained pending after job resolution"),
            VmStep::Error(message) => panic!("workflow failed: {message}"),
            VmStep::Log(message) => panic!("unexpected log: {message}"),
            VmStep::PendingJob { .. } => panic!("unexpected second job"),
        }
    }

    #[test]
    fn rejected_job_rejects_the_workflow() {
        let mut vm = WorkflowVm::new("await runJob('ext', 'job'); return {};", &json!({})).unwrap();

        let pending_id = match vm.step() {
            VmStep::PendingJob { id, .. } => id,
            _ => panic!("expected pending job"),
        };
        vm.reject_job(&pending_id, "job failed").unwrap();
        assert!(matches!(vm.step(), VmStep::Error(message) if message.contains("job failed")));
    }

    #[test]
    fn run_job_requires_extension_and_job_ids() {
        let mut vm = WorkflowVm::new("await runJob('ext'); return {};", &json!({})).unwrap();

        assert!(
            matches!(vm.step(), VmStep::Error(message) if message.contains("requires at least 2 arguments"))
        );
    }
}
